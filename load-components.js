...
function drawSun() {
  ...
  function bindThemeToggleOnce() {
    ...
    // Ensure the correct icon is visible immediately
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    try {
      // Adjust paths if your components are in /components
      ...
      // Notify the rest of the app (functions.js) that components exist
      document.dispatchEvent(new CustomEvent('components:loaded'));
    } catch (err) {
      console.error('[components] injection error:', err);
      // Still dispatch to avoid blocking downstream init that tolerates missing components
      document.dispatchEvent(new CustomEvent('components:loaded'));
    }
  });
